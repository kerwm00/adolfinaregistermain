...
const client = global.client = new Client({
  intents:[
    ...
    presence: {
      activities: [{
        name: beş_config && beş_config.botDurum.length > 0 ? beş_config.botDurum : "Kerem Was Here",
        type: ActivityType.dnd,
        url:""
      }],
      status: 'Kerem Was Here'
    }
  });
    ...
    files.forEach(f => {
        readdir("./src/beş_commands/" + f, (err2, files2) => {
            ...
            files2.forEach(file => {
                ...
                console.log(`🧮 [Kerem - COMMANDS] ${beş_prop.name} Yüklendi!`);
...
readdir("./src/beş_events", (err, files) => {
    ...
    files.filter((file) => file.endsWith(".js")).forEach((file) => {
        ...
        console.log(`📚 [Kerem _ EVENTS] ${beş_prop.conf.name} Yüklendi!`);
    });
});
